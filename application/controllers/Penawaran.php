...
class Penawaran extends CI_Controller
{
	...
	public function index()
	{
		...
		$content = $this->ue_content->select_content_by_type("penawaran");
		...
		if (empty($submit)) {
			...
			$this->load->view('penawaran', ["content" => $content]);
			...
			$this->form_validation->set_rules("officetype", "jenis perusahaan", "trim|required|max_length[255]");
			$this->form_validation->set_rules("estimate", "estimasi jumlah limbah per bulan", "trim|required|max_length[255]");
			$this->form_validation->set_rules("location", "lokasi pengangkutan", "trim|required|max_length[255]");
			$this->form_validation->set_rules("description", "deskripsi limbah", "trim|required|max_length[255]");
