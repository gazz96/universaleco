...
class News extends CI_Controller
{
	...
	{
		if ($page == "") :
			redirect(base_url() . 'news/index/1');
		endif;
		...
		$num = $this->ue_blog->select_blog_list_rows("News", $searchby, $term);

		$config['base_url'] = base_url() . 'news/index';
		$config['first_url'] = base_url() . 'news/index/1/' . $searchby . '/' . $oldterm;
		...
		$blog = $this->ue_blog->select_blog_list("News", $searchby, $term, $limit, $offset);
		...
		$this->load->view('news', ["blog" => $blog, "links" => $link]);
	...
	public function search()
	{
		...
		header("Location: " . base_url() . "news/index/1/title/" . $search);
	}
	...
	public function detail($slug, $id)
	{
		...
		$this->load->view('core/header', ["product_list" => $product_list, "misc_data" => $this->misc_data]);
		$this->load->view('news-detail', ["blog" => $blog]);
		$this->load->view('core/footer', ["certificate_list" => $certificate_list]);
	}
}
