...
class News extends CI_Controller
{
	...
	{
		if ($page == "") :
			redirect(base_url() . 'admin/news/index/1');
		endif;
		...
		$this->session->set_userdata(array("currentpage" => base_url() . "admin/news/index/" . $page . "/" . $sortby . "/" . $sorttype . "/" . $searchlink));
		...
		$num = $this->ue_blog->select_blog_rows("News", $searchby, $term);

		$config['base_url'] = base_url() . 'admin/news/index';
		$config['first_url'] = base_url() . 'admin/news/index/1/' . $sortby . '/' . $sorttype . '/' . $searchby . '/' . $oldterm;
		...
		$news = $this->ue_blog->select_blog("News", $sortby, $sorttype, $searchby, $term, $limit, $offset);
		...
		$this->load->view('admin/news/index', ['news' => $news, 'links' => $link]);
	...
	public function search($sortby = "id", $sorttype = "asc")
	{
		...
		if (empty($searchby)) :
			header("Location: " . base_url() . "admin/news/index/1/" . $sortby . '/' . $sorttype);
		else :
			header("Location: " . base_url() . "admin/news/index/1/" . $sortby . '/' . $sorttype . '/' . $searchby . '/' . $search);
		endif;
	}
	...
	public function add()
	{
		...
		$maintitle = "<h1>Tambah News Baru</h1>";
		...
		if (empty($submit)) :
			...
			$this->load->view('admin/news/_form');
			$this->load->view('admin/core/footer');
		else :
			...
			if ($this->form_validation->run() == FALSE) :
				...
				if ($file == false /*|| $filebg == false*/) :
					...
					$data = [
						...
						'ue_blog_slug'			=> $this->input->post('url'),
					...
					if ($this->ue_blog->insert($data)) :
						$this->session->set_userdata(array('success' => 'News berhasil ditambahkan.'));
						redirect(base_url() . 'admin/news');
					else :
						$this->session->set_userdata(array('err' => 'News gagal ditambahkan karena ada kerusakan sistem.'));
						redirect(base_url() . 'admin/news/add');
					endif;
				endif;
			endif;
	...
	public function detail($id)
	{
		...
		$submit = $this->input->post("submit");
		$maintitle = "<h1>Detail News</h1>";

		$news = $this->ue_blog->select_blog_by_id($id);

		if (empty($submit)) :
			$this->load->view('admin/core/header');
			$this->load->view('admin/core/menu', array("maintitle" => $maintitle));
			$this->load->view('admin/core/tinymce');
			$this->load->view('admin/news/_form', array('news' => $news));
			$this->load->view('admin/core/footer');
		else :
			...
			if ($this->form_validation->run() == FALSE) :
				...
				$image = $_FILES["image"]["name"];
				//$background = $_FILES["background"]["name"];
				$oldimage = $news[0]->ue_blog_image;
				//$oldbg = $news[0]->ue_blog_bg;
				...
				if (!empty($image)) :
					...
					if ($file == false) :
						redirect(base_url() . 'admin/news/detail/' . $id);
						exit();
					else :
				...
				if ($this->ue_blog->update($id, $data)) :
					...
					$this->session->set_userdata(array('success' => 'News berhasil diperbaharui.'));
					redirect($redirect);
				else :
					$this->session->set_userdata(array('err' => 'News gagal diperbaharui karena kerusakan sistem.'));
					redirect(base_url() . 'admin/news/detail/' . $id);
				endif;
			endif;
		endif;
	...
	public function delete($id)
	{
		...
		if (count($blog) > 0) {
			...
			if ($this->ue_blog->delete($id)) {
				...
				$this->session->set_userdata(array("success" => "News berhasil dihapus."));
				redirect($redirect);
			} else {
				$this->session->set_userdata(array("err" => "News gagal dihapus."));
				redirect($redirect);
			}
		} else {
			$this->session->set_userdata(array("err" => "News tidak dapat ditemukan."));
			redirect($redirect);
		}
	}
}
