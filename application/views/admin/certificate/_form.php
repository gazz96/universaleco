...
if (isset($certificate) && count($certificate) > 0) {
  $id = $certificate[0]->ue_certificate_id;
  $name = $certificate[0]->ue_certificate_name;
  $image = $certificate[0]->ue_certificate_icon;
  $description = $certificate[0]->ue_certificate_description;
  $status = $certificate[0]->ue_certificate_status;
}
...
$action = base_url() . "admin/certificate/add";
if ($ur2 == "detail") {
  $action = base_url() . "admin/certificate/detail/" . $ur3;
}
?>
<div class="row">
  <div class="col-md-12">
    <div class="box box-default form-body">
      ...
      <div class="box-body">
        ...
        <form method="post" autocomplete="off" action="<?php echo $action; ?>" enctype="multipart/form-data">
          ...
          <?php
          if (!empty($image) && file_exists("./uploads/certificate/" . $image)) :
          ?>
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label for="image">Gambar Saat Ini</label><br />
                  <img src="<?php echo base_url(); ?>uploads/certificate/<?php echo $image; ?>" alt="" width="350" />
                </div>
              </div>
            </div>
          <?php
          ...
          <div class="box-tools">
            ...
            if (isset($this->session->userdata["currentpage"]) && !empty($this->session->userdata["currentpage"])) {
              ...
              $currentpage = base_url() . "admin/certificate";
            }
            ?>
