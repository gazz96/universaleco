...
$link_name = base_url() . "admin/product/index/" . $ur3 . "/name/desc/" . $ur6 . $search;
$link_status = base_url() . "admin/product/index/" . $ur3 . "/status/desc/" . $ur6 . $search;
...
if (empty($ur4)) {
	...
	$link_name = base_url() . "admin/product/index/" . $ur3 . "/name/asc/" . $ur6 . $search;
} else if ($ur4 == "status" && $ur5 == "asc") {
	$arrow_status = "fa fa-angle-down";
	$link_status = base_url() . "admin/product/index/" . $ur3 . "/status/desc/" . $ur6 . $search;
} else if ($ur4 == "status" && $ur5 == "desc") {
	$arrow_status = "fa fa-angle-up";
	$link_status = base_url() . "admin/product/index/" . $ur3 . "/status/asc/" . $ur6 . $search;
}
?>
...
<div class="row mt40 mb20 header-section">
	...
	<div class="col-md-6" style="text-align: right;">
		<form name="search-form" method="post" action="<?php echo base_url(); ?>admin/product/search/<?php echo $searchlink; ?>">
		...
		<div class="table-responsive">
			<table class="table table-grey">
				...
				?>
						<tr>
							<td><?php echo $x; ?></td>
							<td><?php echo $products->ue_product_name ?></td>
							<td>
								<?php
								if ($products->ue_product_image != null && file_exists("./uploads/product/" . $products->ue_product_image)) :
								?>
									<img src="<?php echo base_url(); ?>uploads/product/<?php echo $products->ue_product_image; ?>" alt="" width="100">
								<?php
								endif;
								?>
							</td>
							<td><?php echo $products->ue_product_status == 1 ? 'Aktif' : 'Tidak Aktif'; ?></td>
							<td align="center">
								<a href="<?php echo base_url(); ?>admin/product/detail/<?php echo $products->ue_product_id; ?>" class="action-link"><i class="fa fa-pencil" aria-hidden="true"></i></a>
							</td>
							<td align="center">
								<a href="<?php echo base_url(); ?>admin/product/delete/<?php echo $products->ue_product_id; ?>" onclick="dialconfirm('<?php echo $products->ue_product_id; ?>');return false;" id="del-button<?php echo $products->ue_product_id; ?>" class="action-link"><i class="fa fa-trash" aria-hidden="true"></i></a>
							</td>
						</tr>
					<?php
						$x++;
					...
					?>
					<tr>
						<td colspan="6" align="center">Produk tidak dapat ditemukan</td>
					</tr>
				<?php
				endif;
				?>
