...
$arrow_source = "fa fa-angle-down";
$arrow_status = "fa fa-angle-down";
$link_name = base_url() . "admin/waste/index/" . $ur3 . "/name/desc/" . $ur6 . $search;
$link_source = base_url() . "admin/waste/index/" . $ur3 . "/source/desc/" . $ur6 . $search;
$link_status = base_url() . "admin/waste/index/" . $ur3 . "/status/desc/" . $ur6 . $search;
...
if (empty($ur4)) {
	...
	$link_name = base_url() . "admin/waste/index/" . $ur3 . "/name/asc/" . $ur6 . $search;
} else if ($ur4 == "source" && $ur5 == "asc") {
	$arrow_source = "fa fa-angle-down";
	$link_source = base_url() . "admin/waste/index/" . $ur3 . "/source/desc/" . $ur6 . $search;
} else if ($ur4 == "source" && $ur5 == "desc") {
	$arrow_source = "fa fa-angle-up";
	$link_source = base_url() . "admin/waste/index/" . $ur3 . "/source/asc/" . $ur6 . $search;
} else if ($ur4 == "status" && $ur5 == "asc") {
	$arrow_status = "fa fa-angle-down";
	$link_status = base_url() . "admin/waste/index/" . $ur3 . "/status/desc/" . $ur6 . $search;
} else if ($ur4 == "status" && $ur5 == "desc") {
	$arrow_status = "fa fa-angle-up";
	$link_status = base_url() . "admin/waste/index/" . $ur3 . "/status/asc/" . $ur6 . $search;
}
?>

<div class="row mt40 mb20 header-section">
	<div class="col-md-6">
		<a href="<?php echo base_url(); ?>admin/waste/add">
			<button class="btn btn-primary blue-bg">Tambah Data Baru</button>
		</a>
	</div>
	<div class="col-md-6" style="text-align: right;">
		<form name="search-form" method="post" action="<?php echo base_url(); ?>admin/waste/search/<?php echo $searchlink; ?>">
			<div style="float: right;">
				<div class="input-group input-group-sm" style="width: 150px; float: left; margin-right: 3px;">
					<select name="searchby" class="form-control pull-right" id="searchby">
						...
						<option value="source" <?php echo $ur6 == "source" ? ' selected="selected"' : ''; ?>>Sumber</option>
					</select>
				</div>
		...
		<div class="table-responsive">
			<table class="table table-grey">
				<tr>
					...
					<th><a href="<?php echo $link_source; ?>">Sumber <i class="<?php echo $arrow_source; ?>"></i></a></th>
					<th>File</th>
				...
				?>
						<tr>
							...
							<td><?php echo $wastes->ue_waste_source; ?></td>
							<td><a href="<?php echo base_url() . "uploads/waste/" . $wastes->ue_waste_url; ?>" target="_blank"><?php echo $wastes->ue_waste_url; ?></a></td>
							<td><?php echo $wastes->ue_waste_status == 1 ? 'Aktif' : 'Tidak Aktif'; ?></td>
							<td align="center">
								<a href="<?php echo base_url(); ?>admin/waste/detail/<?php echo $wastes->ue_waste_id; ?>" class="action-link"><i class="fa fa-pencil" aria-hidden="true"></i></a>
							</td>
							<td align="center">
								<a href="<?php echo base_url(); ?>admin/waste/delete/<?php echo $wastes->ue_waste_id; ?>" onclick="dialconfirm('<?php echo $wastes->ue_waste_id; ?>');return false;" id="del-button<?php echo $wastes->ue_waste_id; ?>" class="action-link"><i class="fa fa-trash" aria-hidden="true"></i></a>
							</td>
						</tr>
					<?php
						$x++;
					...
					?>
					<tr>
						<td colspan="7" align="center">File panduan limbah B3 tidak dapat ditemukan</td>
					</tr>
				<?php
				endif;
				?>
