...
if (isset($blog) && count($blog) > 0) {
  $id = $blog[0]->ue_blog_id;
  $title = $blog[0]->ue_blog_title;
  $date = date("d-m-Y", strtotime($blog[0]->ue_blog_date));
  $url = $blog[0]->ue_blog_slug;
  $author = $blog[0]->ue_blog_author;
  $image = $blog[0]->ue_blog_image;
  $background = $blog[0]->ue_blog_bg;
  $excerpt = $blog[0]->ue_blog_excerpt;
  $description = $blog[0]->ue_blog_description;
  $status = $blog[0]->ue_blog_status;
}
...
$action = base_url() . "admin/blog/add";
if ($ur2 == "detail") {
  $action = base_url() . "admin/blog/detail/" . $ur3;
}
?>
<div class="row">
  <div class="col-md-12">
        ...
        <form method="post" autocomplete="off" action="<?php echo $action; ?>" enctype="multipart/form-data">
          ...
          <div class="box-tools">
            ...
            if (isset($this->session->userdata["currentpage"]) && !empty($this->session->userdata["currentpage"])) {
              ...
              $currentpage = base_url() . "admin/blog";
            }
            ?>
